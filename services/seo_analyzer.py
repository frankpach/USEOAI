...
from models.seo_models import (
    AnalysisRequest, AnalysisResponse, TitleAnalysis, MetaDescription, SemanticSummary, SpeedMetrics, LocalRankCheck,
    GeoRankRequest, GeoRankResponse, KeywordRankResult, Links, HeadingTags
)
from services.semantic_analyzer import SemanticAnalyzer
from config.config import SEOAnalyzerConfig

# --- Utilidad para validar y convertir dicts anidados a modelos Pydantic ---
from pydantic import BaseModel
from typing import Type, Any

def validate_and_convert_dicts(data: dict, model: Type[BaseModel], defaults: dict = None) -> BaseModel:
    """
    Convierte un dict a un modelo Pydantic, rellenando claves faltantes con valores por defecto.
    """
    if defaults is None:
        defaults = {}
...
class BrowserPool:
    """Manages a pool of browser instances for concurrent operations using Playwright"""
    ...
    async def initialize(self):
        """Initialize the browser pool using Playwright"""
        async with self.lock:
            ...
            try:
                ...
                for i in range(self.pool_size):
                    ...
                    logger.info(f"Browser {i+1}/{self.pool_size} initialized (Playwright)")
                self.initialized = True
                logger.info("Browser pool initialized successfully (Playwright)")
    ...
    async def get_page(self, browser: Browser):
        """Get a page from a browser with proper resource management (Playwright)"""
    ...
    async def close(self):
        ...
        async with self.lock:
            ...
            logger.info("Browser pool closed (Playwright)")
...
class SEOAnalyzer:
    ...
    async def analyze_site(self, analysis_request: AnalysisRequest) -> AnalysisResponse:
        """
        Main method to analyze a website with robust error handling.
        ...
        # Fetch HTML content
        try:
            logger.info(f"[SEOAnalyzer] Intentando obtener HTML para {url}")
            # Permitir que el frontend fuerce Playwright solo si está habilitado en config
            if getattr(self.config, 'FORCE_PLAYWRIGHT_FROM_FRONTEND', False) and getattr(analysis_request, 'force_playwright', False):
                logger.info("Frontend ha solicitado forzar Playwright y está permitido por configuración. Usando Playwright para renderizado JS.")
                html, headers, status_code, redirections = await self.scraper._fetch_with_playwright(url)
            else:
                html, headers, status_code, redirections = await self.scraper.fetch_html(url)
            logger.info(f"[SEOAnalyzer] HTML obtenido (status_code={status_code}, longitud={len(html) if html else 0}) para {url}")
        except Exception as e:
            logger.error(f"Failed to fetch HTML for {url}: {e}")
            errors.append(f"Error fetching website: {str(e)}")
            # Use default values for critical data
            html = ""
            headers = {}
            status_code = 0
            redirections = []
        ...
        parsed_data = None
        try:
            logger.info(f"[SEOAnalyzer] Intentando parsear HTML para {url}")
            if self.config.ENABLE_HTML_CACHE and html:
                soup = BeautifulSoup(html, 'lxml')
                self._html_cache[url] = soup

            # Parse HTML using cached soup
            parsed_data = self.scraper.parse_html(html, url)
            logger.info(f"[SEOAnalyzer] Resultado de parse_html: {parsed_data is not None}, keys: {list(parsed_data.keys()) if parsed_data else 'None'}")
            if not parsed_data:
                logger.error(f"Failed to parse HTML for {url}")
                errors.append("Could not parse website content")
                # Create minimal parsed data
                parsed_data = {
                    "title": {"text": "Unknown", "length": 0},
                ...
                logger.info(f"[SEOAnalyzer] Se devolvieron valores por defecto en parsed_data para {url}")
        except Exception as e:
            logger.error(f"HTML parsing failed: {e}")
            errors.append(f"Error parsing website content: {str(e)}")
            # Use default parsed data
            parsed_data = {
                "title": {"text": "Unknown", "length": 0},
                "meta_description": {"text": "", "length": 0},
                "meta_robots": "",
            ...
            logger.info(f"[SEOAnalyzer] Se devolvieron valores por defecto en parsed_data tras excepción para {url}")

        # Asegurar que los datos clave estén presentes en parsed_data
        if 'images_count' not in parsed_data:
            parsed_data['images_count'] = 0
        if 'page_metrics' not in parsed_data:
            parsed_data['page_metrics'] = {}
        if 'links' not in parsed_data:
            parsed_data['links'] = {"internal": [], "external": [], "broken": []}
        if 'broken' not in parsed_data['links']:
            parsed_data['links']['broken'] = []

        # Validar enlaces rotos (si está habilitado)
        try:
            if self.config.ENABLE_BROKEN_LINKS_CHECK:
                parsed_data['links']['broken'] = await self._find_broken_links(parsed_data['links']['internal'] + parsed_data['links']['external'])
            else:
                parsed_data['links']['broken'] = []
        except Exception as e:
            logger.error(f"Broken links check failed: {e}")
            warnings.append(f"Could not check broken links: {str(e)}")
            parsed_data['links']['broken'] = []

        # Incluir images_count y page_metrics en la respuesta AnalysisResponse
        # (puedes agregarlos como campos opcionales si no existen en el modelo)
        # El frontend puede acceder a report.images_count y report.page_metrics
        ...
        try:
            ...
            errors.append(f"Error analyzing title: {str(e)}")
            title_data = {"text": parsed_data["title"]["text"], "length": len(parsed_data["title"]["text"]), "has_keywords": False}
        ...
        try:
            ...
            errors.append(f"Error analyzing performance: {str(e)}")
            speed_metrics = self._get_default_performance_metrics()

        # Check if geolocation analysis should be performed
        should_perform_geo_analysis = (
            (analysis_request.location or (analysis_request.latitude is not None and analysis_request.longitude is not None)) and
            analysis_request.local_radius_km is not None and
            analysis_request.geo_samples is not None
        )

        local_rank = None
        if should_perform_geo_analysis:
            try:
                # Check local ranking
                # Convert keywords to list if it's a string
                keywords_list = None
                if analysis_request.keywords:
                    if isinstance(analysis_request.keywords, str):
                        keywords_list = [analysis_request.keywords]
                    else:
                        keywords_list = analysis_request.keywords
                ...
                local_rank = await self._check_local_ranking(
                    ...
                    analysis_request.latitude,
                    analysis_request.longitude,
                    analysis_request.local_radius_km or 5,
                    analysis_request.geo_samples or 10,
                    keywords_list
                )
            except Exception as e:
                logger.error(f"Local ranking analysis failed: {e}")
                errors.append(f"Error analyzing local ranking: {str(e)}")
                local_rank = self._get_default_local_rank_results(checked=0)
        else:
            # Skip geolocation analysis and use default results
            logger.info("Skipping geolocation analysis - missing required parameters")
            local_rank = self._get_default_local_rank_results(checked=0)

        # --- Generar resumen de errores para el LLM ---
        errores_resumen = {
            'images_without_alt': [img.get('src') for img in parsed_data.get('images_without_alt', []) if img.get('src')],
            'broken_links': parsed_data.get('links', {}).get('broken', []),
            'issues': [],
            'llm_recommendations': []
        }
        # Issues y recomendaciones del análisis previo (si existen)
        if 'issues' in parsed_data and isinstance(parsed_data['issues'], list):
            for issue in parsed_data['issues']:
                errores_resumen['issues'].append({
                    'title': issue.get('title'),
                    'description': issue.get('description'),
                    'priority': issue.get('priority')
                })
        if 'llm_recommendations' in parsed_data and isinstance(parsed_data['llm_recommendations'], list):
            errores_resumen['llm_recommendations'] = parsed_data['llm_recommendations']
        # Preparar textos para el LLM
        try:
            texts = self._prepare_texts_for_semantic_analysis(parsed_data)
            # Llamar al LLM con el resumen de errores
        ...
        if analysis_request.keywords:
            try:
                # Normalizar keywords a lista de str
                if isinstance(analysis_request.keywords, str):
                    keywords_list = [analysis_request.keywords]
                else:
                    keywords_list = analysis_request.keywords
                keyword_usage = self.run_web_keywords_test(
                    url,
                    keywords_list,
                    match="exact"
                )
        ...
        try:
            ...
            recommendations = ["Analysis completed with some errors. Please review the results."]

        # Add error and warning messages to recommendations
        if errors:
            recommendations.insert(0, "⚠️ Some analysis components failed:")
            for error in errors:
                recommendations.append(f"• {error}")

        if warnings:
            recommendations.append("ℹ️ Some components had issues:")
            for warning in warnings:
                recommendations.append(f"• {warning}")

        # Construir la respuesta usando modelos Pydantic con tipos correctos
        try:
            # Extraer campos estructurados del semantic_summary si existen
            executive_summary = semantic_summary.get("executive_summary") if isinstance(semantic_summary, dict) else getattr(semantic_summary, "executive_summary", None)
            technical_score = semantic_summary.get("technical_score") if isinstance(semantic_summary, dict) else getattr(semantic_summary, "technical_score", None)
            onpage_score = semantic_summary.get("onpage_score") if isinstance(semantic_summary, dict) else getattr(semantic_summary, "onpage_score", None)
            offpage_score = semantic_summary.get("offpage_score") if isinstance(semantic_summary, dict) else getattr(semantic_summary, "offpage_score", None)
            overall_score = semantic_summary.get("overall_score") if isinstance(semantic_summary, dict) else getattr(semantic_summary, "overall_score", None)
            issues = semantic_summary.get("issues") if isinstance(semantic_summary, dict) else getattr(semantic_summary, "issues", None)
            llm_recommendations = semantic_summary.get("recommendations") if isinstance(semantic_summary, dict) else getattr(semantic_summary, "recommendations", None)
            checklist = semantic_summary.get("checklist") if isinstance(semantic_summary, dict) else getattr(semantic_summary, "checklist", None)

            # Normalizar h_tags y links para cumplir con los modelos
            h_tags_dict = parsed_data["h_tags"] if isinstance(parsed_data["h_tags"], dict) else {}
            for key in ["h1", "h2", "h3", "h4", "h5", "h6"]:
                if key not in h_tags_dict:
                    h_tags_dict[key] = []
            links_dict = parsed_data["links"] if isinstance(parsed_data["links"], dict) else {}
            for key in ["internal", "external", "broken"]:
                if key not in links_dict:
                    links_dict[key] = []
            # Usar la función para h_tags y links con cast explícito
            h_tags_obj = cast(HeadingTags, validate_and_convert_dicts(parsed_data["h_tags"], HeadingTags, defaults={"h1": [], "h2": [], "h3": [], "h4": [], "h5": [], "h6": []}))
            links_obj = cast(Links, validate_and_convert_dicts(parsed_data["links"], Links, defaults={"internal": [], "external": [], "broken": []}))
            # Asegurar que semantic_structure sea una lista de strings
            semantic_structure = parsed_data["semantic_structure"] if isinstance(parsed_data["semantic_structure"], list) else []
            if isinstance(semantic_structure, dict):
                semantic_structure = list(semantic_structure.values())
            elif not isinstance(semantic_structure, list):
                semantic_structure = []
            ...
            return AnalysisResponse(
                ...
                title=TitleAnalysis(
                    text=str(title_data.get("text", "")),
                    length=int(title_data.get("length", 0)),
                    has_keywords=bool(title_data.get("has_keywords", False))
                ),
                meta_description=MetaDescription(
                    text=str(parsed_data["meta_description"]["text"]),
                    length=int(parsed_data["meta_description"]["length"])
                ),
                meta_robots=parsed_data["meta_robots"],
                canonical_url=parsed_data["canonical_url"],
                h_tags=h_tags_obj,
                paragraphs=parsed_data["paragraphs"],
                semantic_summary=SemanticSummary(**semantic_summary) if isinstance(semantic_summary, dict) else semantic_summary,
                ...
                speed_metrics=SpeedMetrics(
                    ttfb_ms=int(speed_metrics.get("ttfb_ms", 0)),
                    resource_count=int(speed_metrics.get("resource_count", 0)),
                    gzip_enabled=bool(speed_metrics.get("gzip_enabled", False)),
                    lazy_loaded_images=bool(speed_metrics.get("lazy_loaded_images", False))
                ),
                local_rank_check=LocalRankCheck(
                    google_maps=str(local_rank.get("google_maps_rank", "unavailable")),
                    bing_maps=str(local_rank.get("bing_maps_rank", "unavailable")),
                    nap_consistency=bool(local_rank.get("nap_consistency", False))
                ),
                ...
                checklist=checklist,
                # --- Agregar explícitamente los campos para comparación ---
                images_count=parsed_data.get("images_count", 0),
                page_metrics=parsed_data.get("page_metrics", {}),
                # --- Agregar keyword_usage al response ---
                keyword_usage=keyword_usage,
            )
        except Exception as e:
            logger.error(f"Failed to create AnalysisResponse: {e}")
            # En el bloque de retorno de error/fallback, asegurar tipos correctos
            empty_links = Links(internal=[], external=[], broken=[])
            empty_h_tags = HeadingTags(h1=[], h2=[], h3=[], h4=[], h5=[], h6=[])
            empty_semantic_structure = []
            # En el return AnalysisResponse de error:
            return AnalysisResponse(
                status_code=status_code or 0,
                redirections=redirections or [],
                title=TitleAnalysis(text="Error", length=0, has_keywords=False),
                meta_description=MetaDescription(text="", length=0),
                meta_robots="",
                canonical_url="",
                h_tags=empty_h_tags,
                paragraphs=[],
                semantic_summary=SemanticSummary(
                    ...
                    suggested_improvements=["Analysis could not be completed due to errors."]
                ),
                images_without_alt=[],
                links=empty_links,
                semantic_structure=empty_semantic_structure,
                structured_data=[],
                speed_metrics=SpeedMetrics(ttfb_ms=0, resource_count=0, gzip_enabled=False, lazy_loaded_images=False),
                local_rank_check=LocalRankCheck(google_maps="error", bing_maps="error", nap_consistency=False),
                recommendations=["Analysis failed. Please check the URL and try again."],
    ...
    async def _analyze_performance(self, url: str) -> Dict[str, Union[int, bool]]:
        ...
        try:
            ...
            async with response:
                ...
                inline_scripts = len([s for s in soup.find_all('script') if isinstance(s, Tag) and not s.get('src')])
    ...
    async def _check_local_ranking(
        ...
        Args:
            ...
            keywords: Optional list of keywords to use for local search
        ...
        if self.config.ENABLE_GOOGLE_MAPS_CHECK:
            # Use keywords if provided, otherwise use business name
            search_terms = keywords if keywords else [business_name]
            google_results = await self._check_google_maps_ranking(business_name, domain, geo_points, search_terms)

        if self.config.ENABLE_BING_MAPS_CHECK:
            # Use keywords if provided, otherwise use business name
            search_terms = keywords if keywords else [business_name]
            bing_results = await self._check_bing_maps_ranking(business_name, domain, geo_points, search_terms)

        # 4. Extract NAP from website
        nap_data = await self._extract_nap_data(url)

        # 5. Check NAP consistency with maps listing
        profile_data = google_results.get("profile_data", {})
        if isinstance(profile_data, dict):
            nap_consistency = await self._check_nap_consistency(
                business_name, domain, nap_data, profile_data
            )
        else:
            nap_consistency = await self._check_nap_consistency(
                business_name, domain, nap_data, {}
            )
    ...
    async def _check_google_maps_ranking(
        ...
        Args:
            ...
            search_terms: List of search terms to use for Google Maps search
            ...
            async with self.browser_pool.get_browser() as browser:
                ...
                async with self.browser_pool.get_page(browser) as page:
                    try:
                        ...
                        await page.route('**/*', lambda route: route.abort() if route.request.resource_type in self.config.BLOCK_RESOURCE_TYPES else route.continue_())

                        for i, (lat, lon) in enumerate(sample_points):
                            # Prepare search query
                            search_query = "+".join(search_terms) # Combine search terms for Google Maps search
    ...
    async def _check_bing_maps_ranking(
        ...
        Args:
            ...
            domain: Domain to check in results
            geo_points: List of geo points to check from
            search_terms: List of search terms to use for Bing Maps search
        ...
        if bing_maps_key:
            ...
            try:
                for i, (lat, lon) in enumerate(sample_points):
                    query = "+".join(search_terms) # Combine search terms for Bing Maps search
                ...
                return await self._check_bing_maps_scraping(business_name, domain, geo_points, search_terms)
        else:
            # Fallback to scraping if no API key
            return await self._check_bing_maps_scraping(business_name, domain, geo_points, search_terms)

        # Format results
        return self._format_maps_results(ranks, found_count, sample_points)  # type: ignore
            ...
            async with self.browser_pool.get_browser() as browser:
                async with self.browser_pool.get_page(browser) as page:
                    try:
                        ...
                        for i, (lat, lon) in enumerate(sample_points):
                            search_query = "+".join(search_terms) # Combine search terms for Bing Maps search
    ...
    def _get_default_google_maps_result(self) -> Dict[str, Union[str, float, bool, Dict[str, Any]]]:
        ...
        return {
            ...
            "rank_color": "gray",
            "average_rank": 20,  # Changed from None to 0.0
    ...
    async def _check_nap_consistency(
        ...
        if nap_data.get('address') and maps_data.get('address'):
            site_address = self._normalize_address(nap_data['address'] or "")
        ...
        if nap_data.get('phone') and maps_data.get('phone'):
            site_phone = self._normalize_phone(nap_data['phone'] or "")
    ...
    async def analyze_geo_ranking(self, request: GeoRankRequest) -> GeoRankResponse:
        """
        Perform standalone geolocation-based ranking analysis.

        Args:
            request: GeoRankRequest object containing analysis parameters

        Returns:
            GeoRankResponse object with detailed ranking analysis
        """
        logger.info(f"Starting geo-ranking analysis for {request.company_name}")

        # Validate that keywords are provided and not empty
        if not request.keywords or len(request.keywords) == 0:
            raise ValueError("Keywords are required for geo-ranking analysis")

        # Validate that each keyword is not empty
        valid_keywords = [kw.strip() for kw in request.keywords if kw and kw.strip()]
        if not valid_keywords:
            raise ValueError("At least one valid keyword is required for geo-ranking analysis")

        # Update request with cleaned keywords
        request.keywords = valid_keywords

        # Get coordinates from location or use provided coordinates
        ...
        if not coords:
            raise ValueError("Could not determine coordinates from location or provided coordinates")

        # Generate geo samples around the location
        geo_points = self._generate_geosamples(
            coords[0], coords[1],
            request.local_radius_km, "km",
            request.geo_samples
        )
        ...
        for keyword in request.keywords:
            ...
            # Check Google Maps ranking for this keyword
            google_results = {}
            if self.config.ENABLE_GOOGLE_MAPS_CHECK:
                google_results = await self._check_google_maps_ranking(
                    request.company_name,
                    "",  # No domain needed for standalone analysis
                    geo_points,
                    [keyword] # Pass keyword to _check_google_maps_ranking
                )

            # Check Bing Maps ranking for this keyword
            bing_results = {}
            if self.config.ENABLE_BING_MAPS_CHECK:
                bing_results = await self._check_bing_maps_ranking(
                    request.company_name,
                    "",  # No domain needed for standalone analysis
                    geo_points,
                    [keyword] # Pass keyword to _check_bing_maps_ranking
                )

            # Calculate coverage and visibility metrics
            google_coverage = float(google_results.get("coverage_percentage", 0.0))
            bing_coverage = float(bing_results.get("coverage_percentage", 0.0))

            # Average coverage across platforms
            coverage_percentage = (google_coverage + bing_coverage) / 2.0 if (google_coverage + bing_coverage) > 0 else 0.0

            # Calculate visibility score (0-100)
            visibility_score = min(100.0, coverage_percentage * 2.0)  # Scale coverage to 0-100

            # Check for NAP inconsistencies
            nap_inconsistencies = []
            if google_results.get("profile_data"):
                profile_data = google_results["profile_data"]
                if isinstance(profile_data, dict):
                    # Check for basic NAP inconsistencies
                    if profile_data.get("address") and not profile_data.get("phone"):
                        nap_inconsistencies.append("Missing phone number in Google Maps listing")
                    if profile_data.get("phone") and not profile_data.get("address"):
                        nap_inconsistencies.append("Missing address in Google Maps listing")

            # Create keyword result
            keyword_result = KeywordRankResult(
                keyword=keyword,
                average_rank=float(google_results.get("average_rank")) if isinstance(google_results.get("average_rank"), (int, float)) else None,
                coverage_percentage=coverage_percentage,
                total_samples=len(geo_points),
                found_in_samples=int((coverage_percentage / 100.0) * len(geo_points)),
                google_maps_rank=str(google_results.get("rank_text")) if google_results.get("rank_text") else None,
                bing_maps_rank=str(bing_results.get("rank_text")) if bing_results.get("rank_text") else None,
                nap_inconsistencies=nap_inconsistencies,
                visibility_score=visibility_score
            )

            keyword_results.append(keyword_result)
            total_visibility_score += visibility_score

        # Calculate overall metrics
        overall_visibility_score = total_visibility_score / len(request.keywords) if request.keywords else 0.0

        # Check NAP consistency (use first keyword's results as reference)
        nap_consistency = False
        has_verified_listing = False

        if keyword_results and google_results.get("profile_data"):
            profile_data = google_results["profile_data"]
            if isinstance(profile_data, dict):
                has_verified_listing = bool(google_results.get("is_verified", False))
                # Basic NAP consistency check
                nap_consistency = bool(
                    profile_data.get("address") and
                    profile_data.get("phone") and
                    profile_data.get("title")
                )

        # Create response
        location_used = request.location or f"{coords[0]:.4f}, {coords[1]:.4f}"

        return GeoRankResponse(
            company_name=request.company_name,
            location_used=location_used,
            coordinates=coords,
            radius_km=request.local_radius_km,
            total_samples=len(geo_points),
            keywords_analyzed=len(request.keywords),
            keyword_results=keyword_results,
            overall_visibility_score=overall_visibility_score,
            nap_consistency=nap_consistency,
            has_verified_listing=has_verified_listing,
            analysis_timestamp=time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime())
        )

    def run_web_keywords_test(self, url: str, keywords: List[str], match: str = "exact") -> Dict:
        """
        Analiza el uso de palabras clave en <h1>-<h6> y <p> de una página web.
        Args:
            url: URL de la página a analizar
            keywords: Lista de palabras clave objetivo
            match: 'exact' o 'partial' para coincidencia exacta o parcial
        Returns:
            Dict con resultados por keyword y sugerencias
        """
        import requests
        ...
        # Obtener HTML (usar requests, fallback a scraper si falla)
        try:
            resp = requests.get(url, headers={"User-Agent": self.user_agent}, timeout=10)
            html = resp.text
        except Exception:
            html, _, _, _ = self.scraper.fetch_html(url)

        soup = BeautifulSoup(html, 'lxml')
        # Extraer headers y párrafos
        headers = []
        for level in range(1, 7):
        ...
        for kw in keywords:
            kw_lc = kw.lower()
            used_in_headers = False
            used_in_paragraphs = False
            freq_headers = 0
            freq_paragraphs = 0
            positions_headers = []
            positions_paragraphs = []
            overused = False
            ...
            for h in headers:
                text = h["text"].lower()
                if match == "partial":
                    found = kw_lc in text or any(kw_lc[:max(3, len(kw_lc)//2)] in word for word in text.split())
                else:
                    found = kw_lc in text.split()
                count = text.count(kw_lc) if match == "exact" else text.count(kw_lc)
                if found:
                    used_in_headers = True
                    freq_headers += count if count > 0 else 1
                    # Posición: inicio, medio, final
                    if text.startswith(kw_lc):
                        positions_headers.append("start")
                    elif text.endswith(kw_lc):
                        positions_headers.append("end")
                    else:
                        positions_headers.append("middle")
            # Párrafos
            for p in paragraphs:
                text = p.lower()
                if match == "partial":
                ...
                count = text.count(kw_lc) if match == "exact" else text.count(kw_lc)
                if found:
                    used_in_paragraphs = True
                    freq_paragraphs += count if count > 0 else 1
                    if text.startswith(kw_lc):
                        positions_paragraphs.append("start")
                    elif text.endswith(kw_lc):
                        positions_paragraphs.append("end")
                    else:
                        positions_paragraphs.append("middle")
            # Sobreuso
            if freq_headers > 3 or freq_paragraphs > 3:
                overused = True
            # Observación y color
            if used_in_headers and used_in_paragraphs:
                color = "green"
                obs = "Correcto: Usada en headers y párrafos."
            elif used_in_paragraphs:
                color = "yellow"
                obs = "Sugerido: Usar también en encabezados."
            elif used_in_headers:
                color = "yellow"
                obs = "Sugerido: Usar también en párrafos."
            else:
                color = "red"
                obs = "No usada."
            if overused:
                obs += " ⚠️ Sobreuso (>3 repeticiones en un bloque)"
                color = "orange"
            results.append({
                "keyword": kw,
                "h1_h6": used_in_headers,
                "p": used_in_paragraphs,
                "freq": freq_headers + freq_paragraphs,
                "obs": obs,
                "color": color,
                "freq_headers": freq_headers,
                "freq_paragraphs": freq_paragraphs,
                "positions_headers": positions_headers,
                "positions_paragraphs": positions_paragraphs
            })
        # Checklist
        checklist = [
            "Revisar ubicación estratégica de keywords"
        ]
        return {
            "results": results,
            "checklist": checklist,
